import { lat2tile, lon2tile, tile2lat, tile2lon, degreesToRadians, radiansToDegrees } from './math';
...
export const getProjectedIndicesFromBounds = (
	[south, west, north, east]: [number, number, number, number],
	domain: Domain,
	xPrecision: number,
	yPrecision: number
	...
	let [s, w, n, e] = getRotatedSWNE(domain, projection, [south, west, north, east]);

	const dx = projectionGrid.dx;
	const dy = projectionGrid.dy;
...
export const getIndicesFromBounds = (
	...
	if (domain.grid.projection) {
		return getProjectedIndicesFromBounds(
